...
int C_BaseEntity::GetSequenceActivity(int sequence)
{
	...
	const auto hdr = g_pMdlInfo->GetStudioModel(model);
	if (!hdr)
		return -1;

	static auto offset = (DWORD)g_pUtils->FindPatternIDA("client.dll", "55 8B EC 83 7D 08 FF 56 8B F1 74 3D");
...
void C_BaseEntity::SetAngle2(Vector wantedang)
{
	...
	static SetAngleFn SetAngle2 = (SetAngleFn)((DWORD)g_pUtils->FindPattern("client.dll", (PBYTE)"\x55\x8B\xEC\x83\xE4\xF8\x83\xEC\x64\x53\x56\x57\x8B\xF1", "xxxxxxxxxxxxxx"));
...
void IEngineTrace::TraceRay(const Ray_t &ray, unsigned int fMask, ITraceFilter* filter, trace_t *trace)
{
	typedef void(__thiscall* fn)(void*, const Ray_t&, unsigned int, ITraceFilter*, trace_t*);
	return VFunc<fn>(this, 5)(this, ray, fMask, filter, trace);
}